#!/usr/bin/env python3
"""
harvest_discrimination.py — the discrimination wire: who is treated unequally,
by whom, under what law, and what is done about it.

After the Discrimination section of Suppression, which names the forms directly:
sexism, racism, ethnicism, LGBTQIA+-phobia, classism, religious discrimination,
intellectualism, ableism, dwarfism discrimination, speciesism, and so on. It
carries the section's own two structural claims alongside those: that states
lack effective machinery to stop discriminatory actors infiltrating and
hijacking institutions — the protection existing on the books and in background
disclosures rather than in practice — and that leaders who are not themselves
discriminatory still exploit these divisions to gain power.

An opinion column is not this subject, and neither is the word in its other
senses — a discriminating palate, price discrimination, discriminant analysis.
Who is treated unequally, and what follows from it, is. Every subject term
...
# --------------------------------------------------------------------------
USER_AGENT = ("Mozilla/5.0 (compatible; discrimination-feed/1.0; "
...
# THE SUBJECTS
#
# The ten forms the Discrimination section names, plus the mechanisms it
# describes: infiltration of institutions, division used as a route to power,
# law that exists on the books but is not enforced, and how any of it is
# measured.
#
# Each subject is (term, context) pairs. The term must appear AND one of its
# context words, so an opinion column or the word in its other senses cannot
# reach the wire.
# --------------------------------------------------------------------------
TOPICS = [
    ("sexism", "Sexism and women's rights", [
        ("sexism", []), ("sexist", ["prejudice", "attitudes", "policy", "law", "remarks", "culture"]),
        ("misogyny", []), ("gender discrimination", []), ("sex discrimination", []),
        ("economic rights", ["women", "equal", "unequal"]),
        ("women", ["denied", "barred", "excluded", "cannot", "unequal rights", "legal barriers",
                   "guardianship", "inherit", "own property", "open a bank"]),
        ("guardianship law", []), ("female genital mutilation", []), ("child marriage", []),
        ("harassment", ["workplace", "ruling", "tribunal", "systemic", "inquiry"]),
        ("maternity", ["discrimination", "dismissed", "denied", "penalty"]),
        ("gender quota", []), ("equal rights amendment", []),
    ]),
    ("racism", "Racism and racial discrimination", [
        ("racism", []), ("racial discrimination", []), ("racial profiling", []),
        ("racial segregation", []), ("apartheid", []), ("colour bar", []),
        ("racial bias", ["study", "found", "systemic", "algorithm", "hiring", "sentencing"]),
        ("cerd", []), ("committee on the elimination of racial discrimination", []),
        ("affirmative action", []), ("reparations", ["slavery", "colonial", "racial", "descendants"]),
        ("racial inequality", []), ("systemic racism", []),
        ("hate speech", ["racial", "racist", "law", "prosecution", "platform"]),
    ]),
    ("ethnicity", "Ethnicity, skin colour and language", [
        ("ethnic discrimination", []), ("ethnic minorit*", ["discriminat*", "excluded", "denied", "persecut*",
                             "detained", "targeted", "segregat*", "barred", "evict*"]),
        ("skin colour", ["discriminat*", "bias", "preference"]),
        ("skin color", ["discriminat*", "bias", "preference"]),
        ("colourism", []), ("colorism", []),
        ("language discrimination", []), ("linguistic minority", []),
        ("mother tongue", ["banned", "denied", "excluded", "rights", "policy"]),
        ("indigenous", ["discriminat*", "denied", "excluded", "rights violation", "recognition"]),
        ("ethnic cleansing", []), ("statelessness", []), ("roma", ["discriminat*", "evict*", "segregat*", "excluded"]),
        ("caste", ["discriminat*", "based", "system", "atrocities", "reservation"]),
    ]),
    ("lgbtqia", "LGBTQIA+ people and the law", [
        ("criminalise homosexuality", []), ("criminalize homosexuality", []),
        ("same-sex", ["ban", "law", "ruling", "criminal", "marriage", "rights", "recognition"]),
        ("lgbt", ["rights", "law", "ban", "ruling", "discriminat*", "persecut*", "protest", "attack"]),
        ("lgbtq", ["rights", "law", "ban", "ruling", "discriminat*", "persecut*"]),
        ("anti-gay", []), ("homophobia", []), ("transphobia", []),
        ("gender recognition", []), ("conversion therapy", []),
        ("pride", ["banned", "attacked", "march", "police", "permit"]),
        ("decriminalis*", ["homosexuality", "same-sex"]), ("decriminaliz*", ["homosexuality", "same-sex"]),
    ]),
    ("classism", "Class and social origin", [
        ("classism", []), ("class discrimination", []), ("social origin", ["discriminat*", "protected"]),
        ("class ceiling", []), ("socioeconomic discrimination", []),
        ("poverty", ["stigma", "penalis*", "penaliz*", "criminalis*", "criminaliz*", "discriminat*"]),
        ("homeless", ["criminalis*", "criminaliz*", "banned", "fined", "moved on", "discriminat*"]),
        ("accent", ["discriminat*", "bias", "prejudice"]),
        ("postcode", ["discriminat*", "bias", "denied"]),
    ]),
    ("religion", "Religious freedom and persecution", [
        ("religious freedom", []), ("religious discrimination", []), ("religious persecution", []),
        ("blasphemy", ["law", "charge", "conviction", "sentence", "arrest"]),
        ("apostasy", []), ("freedom of belief", []), ("forced conversion", []),
        ("place of worship", ["attack", "demolish", "closed", "banned", "permit"]),
        ("headscarf", ["ban", "ruling", "law"]), ("hijab", ["ban", "ruling", "law", "protest"]),
        ("antisemitism", []), ("islamophobia", []),
        ("religious minority", ["persecut*", "discriminat*", "attack", "detained", "denied"]),
        ("special rapporteur", ["religion", "belief"]),
    ]),
    ("ableism", "Disability and ableism", [
        ("ableism", []), ("disability discrimination", []), ("disability rights", []),
        ("institutionalis*", ["disabled", "disability", "conditions", "children"]),
        ("institutionaliz*", ["disabled", "disability", "conditions", "children"]),
        ("forced sterilisation", []), ("forced sterilization", []),
        ("forced contraception", []),
        ("accessibility", ["failure", "law", "ruling", "denied", "barrier", "complaint"]),
        ("inaccessible", ["building", "transport", "service", "polling", "school"]),
        ("reasonable adjustment", []), ("reasonable accommodation", []),
        ("crpd", []), ("convention on the rights of persons with disabilities", []),
        ("immigration", ["disability", "medical inadmissibility", "health grounds", "excluded"]),
        ("guardianship", ["disabled", "removed", "capacity", "restored"]),
    ]),
    ("intellectual", "Intellectual discrimination", [
        ("intellectual discrimination", []), ("intellectualism", ["discriminat*", "elitis"]),
        ("anti-intellectual*", []), ("eugenic", []),
        ("iq", ["ranking", "discriminat*", "policy", "hierarch", "race"]),
        ("cognitive", ["discriminat*", "excluded", "capacity", "testing", "denied"]),
        ("neurodiver*", []), ("learning disability", ["excluded", "denied", "discriminat*", "segregat*"]),
        ("survival of the fittest", []), ("social darwinis", []),
        ("meritocracy", ["myth", "critique", "challenged", "questioned"]),
    ]),
    ("stature", "Stature and appearance", [
        ("dwarfism", []), ("little people", ["discriminat*", "rights", "abuse"]),
        ("height discrimination", []), ("appearance discrimination", []),
        ("lookism", []), ("body size", ["discriminat*", "bias", "protected"]),
        ("weight discrimination", []), ("dwarf tossing", []),
    ]),
    ("speciesism", "Speciesism and animal standing", [
        ("speciesism", []), ("animal sentience", []), ("sentience", ["recognis*", "recogniz*", "law", "bill", "legal"]),
        ("animal rights", ["law", "ruling", "constitution", "legal", "recognition", "court"]),
        ("animal welfare", ["law", "bill", "constitution", "ruling", "standard"]),
        ("legal personhood", ["animal", "river", "nature", "great ape", "elephant"]),
        ("rights of nature", []), ("constitution", ["animal", "nature", "sentience"]),
        ("great ape", ["rights", "personhood", "court"]),
    ]),
    ("age", "Age discrimination", [
        ("ageism", []), ("age discrimination", []),
        ("older workers", ["discriminat*", "excluded", "dismissed", "barrier"]),
        ("mandatory retirement", []), ("young people", ["excluded", "denied", "discriminat*", "barred"]),
        ("elder abuse", []),
    ]),
    ("migrants", "Nationality and migration status", [
        ("xenophobia", []), ("nationality discrimination", []),
        ("migrant", ["discriminat*", "excluded", "denied", "detained", "deport", "rights"]),
        ("refugee", ["discriminat*", "excluded", "denied", "pushback", "rights"]),
        ("asylum", ["denied", "rejected", "policy", "restriction", "detention"]),
        ("hostile environment", []), ("right to work", ["migrant", "asylum", "denied"]),
        ("citizenship", ["stripped", "denied", "revoked", "discriminat*", "test"]),
    ]),
    ("work", "Discrimination at work", [
        ("hiring discrimination", []), ("employment discrimination", []),
        ("workplace discrimination", []), ("recruitment bias", []),
        ("cv study", []), ("audit study", ["discriminat*", "hiring", "callback"]),
        ("callback", ["discriminat*", "study", "names", "gap"]),
        ("dismissal", ["discriminat*", "tribunal", "unfair", "ruling"]),
        ("equality tribunal", []), ("employment tribunal", ["discriminat*", "ruling", "award"]),
        ("pay discrimination", []), ("promotion", ["discriminat*", "barrier", "passed over", "bias"]),
    ]),
    ("services", "Access to housing, health and services", [
        ("housing discrimination", []), ("rental discrimination", []),
        ("mortgage discrimination", []), ("redlining", []),
        ("healthcare discrimination", []), ("medical bias", []),
        ("denied service", []), ("refused service", []),
        ("segregat*", ["school*", "class*", "pupil*", "student*", "housing", "hospital*",
                       "service*", "ward*", "unit*", "seating", "neighbourhood*"]),
        ("access denied", ["disabled", "minority", "religious", "migrant"]),
        ("public accommodation", []),
    ]),
    ("violence", "Hate crime and targeted violence", [
        ("hate crime", []), ("hate-motivated", []), ("bias-motivated", []),
        ("mob violence", ["minority", "religious", "ethnic", "caste"]),
        ("lynching", []), ("pogrom", []),
        ("attack", ["mosque", "synagogue", "church", "temple", "minority", "lgbt", "migrant"]),
        ("incitement", ["hatred", "violence", "genocide"]),
        ("genocide", ["warning", "risk", "ruling", "recognition", "determination", "prevention"]),
        ("persecution", ["minority", "religious", "ethnic", "systematic"]),
    ]),
    ("capture", "Discriminatory actors inside institutions", [
        ("infiltrat*", ["institution", "police", "military", "government", "party", "agency"]),
        ("extremist", ["officer", "official", "member", "vetting", "infiltrat*", "inside"]),
        ("vetting", ["failure", "background", "extremis", "officer", "recruit"]),
        ("far-right", ["network", "inside", "police", "military", "official", "infiltrat*"]),
        ("supremacis*", ["network", "officer", "member", "inside", "organis*", "organiz*"]),
        ("institutional", ["racism", "sexism", "discrimination", "failure", "review"]),
        ("whistleblower", ["discrimination", "racism", "harassment", "retaliation"]),
        ("inquiry", ["institutional racism", "discrimination", "misconduct", "culture"]),
    ]),
    ("politics", "Division as a route to power", [
        ("scapegoat", []), ("dog whistle", []), ("divisive rhetoric", []),
        ("campaign", ["anti-migrant", "anti-lgbt", "racist", "sectarian", "divisive"]),
        ("populis*", ["minority", "migrant", "divisive", "scapegoat", "rhetoric"]),
        ("culture war", ["policy", "legislation", "campaign", "ban"]),
        ("election", ["ethnic", "sectarian", "communal", "divisive", "minority vote"]),
        ("disinformation", ["minority", "migrant", "religious", "ethnic", "targeted"]),
    ]),
    ("law", "Anti-discrimination law and enforcement", [
        ("anti-discrimination", []), ("equality act", []), ("equality law", []),
        ("equality body", []), ("equality commission", []),
        ("discrimination case", []), ("discrimination ruling", []), ("discrimination complaint", []),
        ("protected characteristic", []), ("equal treatment directive", []),
        ("enforcement", ["discrimination", "equality", "weak", "gap", "failure", "body"]),
        ("on the books", []), ("unenforced", []),
        ("universal periodic review", []), ("treaty body", ["discrimination", "review", "finding"]),
        ("rollback", ["equality", "rights", "protection", "anti-discrimination", "dei"]),
        ("diversity programme", ["scrapped", "cut", "banned", "rollback"]),
        ("diversity program", ["scrapped", "cut", "banned", "rollback"]),
    ]),
    ("data", "Measuring who is discriminated against", [
        ("discrimination survey", []), ("equality data", []), ("disaggregated data", []),
        ("prevalence", ["discrimination", "prejudice", "bias"]),
        ("index", ["discrimination", "equality", "inclusion", "rights", "freedom"]),
        ("survey", ["discrimination", "prejudice", "attitudes", "bias", "experienced"]),
        ("study", ["discrimination", "prejudice", "bias", "found that", "gap"]),
        ("report", ["discrimination", "equality", "minority rights", "religious freedom"]),
        ("algorithmic bias", []), ("automated decision", ["bias", "discriminat*", "challenged"]),
    ]),
    ("resistance", "What is set against it", [
        ("anti-racism", []), ("civil rights movement", []), ("equality campaign", []),
        ("class action", ["discrimination", "equal pay", "bias"]),
        ("landmark ruling", ["discrimination", "equality", "rights"]),
        ("quota", ["representation", "parliament", "board", "reserved"]),
        ("reserved seats", []), ("affirmative", ["action", "measures", "programme", "program"]),
        ("truth commission", []), ("apology", ["state", "official", "historic", "victims"]),
        ("protest", ["discrimination", "racism", "rights", "equality", "minority"]),
    ]),
]

ANCHOR = [
    # Discrimination-as-a-system language. An ordinary crime report or an
    # opinion column about a culture-war topic is not this subject; who is
    # treated unequally, by whom, under what law, and what is done about it, is.
    "discrimination", "discriminatory", "racism", "racist", "sexism", "misogyny",
    "xenophobia", "homophobia", "transphobia", "antisemitism", "islamophobia",
    "ableism", "ageism", "classism", "colourism", "colorism", "speciesism",
    "racial profiling", "ethnic minority", "religious minority", "religious freedom",
    "religious persecution", "blasphemy law", "apostasy",
    "criminalise homosexuality", "criminalize homosexuality", "anti-gay",
    "gender recognition", "conversion therapy", "same-sex marriage",
    "disability rights", "forced sterilisation", "forced sterilization",
    "accessibility", "reasonable adjustment", "reasonable accommodation",
    "hate crime", "hate speech", "incitement to hatred", "persecution",
    "ethnic cleansing", "genocide", "apartheid", "segregation", "statelessness",
    "anti-discrimination", "equality act", "equality commission", "equality body",
    "protected characteristic", "affirmative action", "reparations",
    "institutional racism", "systemic racism", "algorithmic bias",
    "animal sentience", "rights of nature", "legal personhood",
    "hiring discrimination", "employment discrimination", "housing discrimination",
    "redlining", "equal pay", "child marriage", "female genital mutilation",
    "universal periodic review", "special rapporteur", "cerd", "crpd",
]

BLOCK = [
    # opinion and entertainment that uses the vocabulary without being about
    # anyone's treatment
    "film review", "movie review", "tv review", "video game", "reality show",
    "celebrity feud", "red carpet", "awards snub", "box office",
    "sports rivalry", "transfer rumour", "match report", "fantasy league",
    "horoscope", "recipe", "gift guide", "black friday", "coupon",
    "sponsored content", "press release", "partner content",
    # the taste-and-preference sense of the words
    "discriminating palate", "discriminating taste", "discerning buyer",
    "price discrimination", "discriminate between wavelengths",
    "discriminating power of the test", "discriminant analysis",
    # other wires
    "stock market", "earnings beat", "share price", "quarterly results",
]
...
DECIDED = [
    ...
    "decriminalised", "decriminalized", "criminalised", "criminalized",
    "convicted", "acquitted", "fined", "ordered", "settled", "compensation awarded",
]
INSTITUTIONAL = [
    "united nations", "human rights council", "ohchr", "special rapporteur",
    "treaty body", "universal periodic review", "european court of human rights",
    "inter-american court", "african commission", "equality commission",
    "equality body", "ombudsman", "national human rights institution",
    "fundamental rights agency", "council of europe", "ecri",
    "committee on the elimination of racial discrimination", "cedaw", "crpd",
    "official gazette", "court filing", "parliamentary committee",
    "peer-reviewed", "published in", "study finds", "working paper", "dataset",
    "official data", "government figures", "national statistics", "census",
]
MEASURED = [
    "per cent", "percent", "%", "one in", "1 in", "million", "billion",
    "figures show", "fell by", "rose by", "increase of", "decrease of",
    "estimated", "median", "average", "ranked", "index", "prevalence",
    "share of", "rate of", "countries still", "number of countries",
]
PENDING = [
    ...
    "green paper", "white paper", "inquiry launched",
]
...
def run(dry_run=False, fixtures=None):
    ...
    payload = {
        ...
        "languages": languages,
        "standings": [
            {"id": "official", "label": "Courts, treaty bodies & equality commissions"},
            {"id": "research", "label": "Research & rights monitoring"},
            {"id": "press", "label": "Press"},
            {"id": "rights", "label": "Rights & community organisations"},
        ],
    ...
    if dry_run:
        print("\n--dry-run: wire_discrimination.json not written")
        return payload
